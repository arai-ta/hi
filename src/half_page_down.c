...
/**********************************************************************/
/*  半ページスクロール関数(下) - half_page_down                       */
/* ------------------------------------------------------------------ */
/* 関数概要                                                           */
/*   半ページ単位のスクロール(下)を行う。                             */
/**********************************************************************/
vd  half_page_down( struct ifa_s *ifa ) /* 半ページスクロール関数(下) */
{
    li scr_end = 0;
    li half_page = (ifa->lines - (BASE_Y + 2) ) / 2 ;
    trace_start( ifa, "half_page_down" ) ;
                                        /* トレース取得               */
/* ------------------------------------------------------------------ */
/* ページのスクロール                                                 */
/* ------------------------------------------------------------------ */
    ifa->data_offset += ( half_page * 16 ) ;
                                        /* データオフセットの更新     */
